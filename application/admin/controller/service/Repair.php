...
class Repair extends Backend {
    ...
    public function add() {
        if ($this->request->isPost()) {
            ...
            $params['repair_user_id'] = $admin['id'];
            $params['from_srv'] = 2; //后台管理人员
    ...
    private function handleSearch($searchfields=null) {
        ...
        $list = $this->model->with('community,user,admin')->where($where)->order($orderParams)->limit($offset, $limit)->select();
