...
class Player extends Backend {
    ...
    //检索时匹配的字段
    protected $searchfields = 'nickname,number';
    ...
    public function add() {
        if ($this->request->isPost()) {
            ...
            $params['votetime'] = strtotime($params['votetime']);
    ...
    private function handleSearch($searchfields=null) {
        ...
        $list = $this->model->with('subject,user')->where($where)->order($sort, $order)->limit($offset, $limit)->select();
